...
>
> **2026-09-21 적용(현행):** 존버를 **코드로 그린다**(`lib/zonber_painter.dart`) — 별의 커비 느낌의 동그란 몸 + 얼굴(눈·볼·입) + 양손 + 양발.
> **표정이 곧 캐릭터다**(2026-09-23) — 몸 색만 다르면 스킨과 구분이 안 되므로 캐릭터마다 평소·아야·신남 셋이 전부 다르다:
> 민트 무심 · 잽 팔팔 · 루나 졸림 · **블레이즈 뾰로통(볼터치)** · 써니 해맑음 · **레이스 고혹(긴 속눈썹·입술)** · 체리 사랑스러움(속눈썹·하트 눈) · 코코 아기(큰 눈·ω 입).
> 부품은 `_FacePen`(눈·눈썹·입·볼터치·속눈썹·하트 눈·눈물)에 모아 두고 캐릭터별로 조합한다.
> 맞음/실점이면 아야+납작, 아슬아슬 회피/세이브면 신남. 몸은 돌지 않고 가는 쪽으로 살짝 기운다.
...
| `cyber_red` | Blaze / 블레이즈 | 토마토 레드 `#F2503D` | 작은 불꽃 머리 | 볼이 발그레한 승부사. 지면 뾰로통 | 세 존 모두 60초 |
| `solar_gold` | Sunny / 써니 | 해바라기 `#FFC928` | 해 모양 왕관 테두리 | 맞아도 웃는 햇살 | 명패(브론즈 이상) 획득 |
| `void_dark` | Wraith / 레이스 | 먹색 `#3A3F4B` + 은은한 은빛 가장자리 | 뾰족한 두건 끝 | 긴 속눈썹으로 조용히 바라보는 밤 | 명패(골드 이상) 획득 |
| `blossom_pink` | Cherry / 체리 | 벚꽃 분홍 `#FF7FB0` | 꽃잎 한 장 | 보여지는 걸 좋아한다. 피하면서도 포즈 | 누적 100판(`b_runs_100`) |
| `frost_cyan` | Coco / 코코 | 하늘 시안 `#35C9E8` | 물방울 더듬이 | 제일 막내. 큰 눈에 앙 다문 ω 입 | 누적 1시간(`b_time_1h`) |
...
| 파일 | 크기 | 내용 |
|---|---|---|
| 몸통 × 6 | 512×512, 투명 | 얼굴 없는 몸통. 몸통이 캔버스의 약 80%를 채우고 가운데 정렬 |
| 장비 소품 | 512×512, 투명 | 몸통과 같은 캔버스 기준 위치에 소품만(파일명 = 장비 id) |

> 2026-09-22: 옛 원판 이미지(`assets/images/characters/`)와 1차 AI 아트(`assets/images/game/`·`GameArt`)를 모두 제거했다. 지금은 전부 코드 드로잉이고, 위 규격은 2차 아트 요청용 계획이다(받으면 읽기 코드부터 다시 만든다).
