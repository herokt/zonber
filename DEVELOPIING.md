...
[x] Top 10 리더보드 출력 UI
[x] Top 30 확장 및 내 랭킹 표시 기능 추가 (Phase 6 항목 선행 구현)
[x] 실기기 테스트 및 크래시 해결 완료

Phase 3: 맵 에디터 (UGC) - [완료]
[x] 그리드 기반 맵 에디터 UI 개발 (Main Menu & Editor Mode)
[x] 유저 커스텀 맵 저장/불러오기 (grid_data)
[x] 맵 제작 3대 원칙 검증 로직 구현 (30초 생존 검증, 원칙 1/2는 자율)

Phase 4: 폴리싱 & 출시 - [진행 중]
[x] 사운드 (BGM, SFX) 적용 및 설정(Settings) 메뉴 구현
[x] 진동 (Haptic Feedback) 적용 (Hit/Death)
...
[x] **완벽한 충돌 처리 (Collision Robustness)**
    - [x] 플레이어가 장애물 내부로 절대 침범하지 못하도록 물리 보완 (Tunnelling 방지)
    - [x] 탄알이 장애물 내부로 뚫고 들어가는 현상 수정 (도탄 시점 보정)

[ ] **장애물 고도화**
    - [x] 장애물 생성 시 서로 겹치지 않게 배치 (Non-overlapping Spawning)
...
[x] **랭킹 시스템 확장** (Phase 2로 앞당겨 구현됨)
    - [x] 랭킹 전용 페이지 생성 (맵 선택 화면 등에서 진입)
    - [x] Top 30위까지 표시 확장
    - [x] 내 순위가 30위 밖일 때도 내 등수/기록 표시 기능 추가
